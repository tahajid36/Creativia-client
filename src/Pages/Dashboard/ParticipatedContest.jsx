  ...
  const {data: participated = [], isLoading} = useQuery({
    ...
    queryFn: async () =>{
      const result = await axios(
        `${import.meta.env.VITE_API_URL}/participations/${user?.email}`
      )
      return result.data
    }
  })
      ...
      <div className="w-full max-w-[1200px] mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 justify-between items-start lg:items-end">
          <div className="flex flex-col gap-2">
            <h1 className="text-white text-4xl font-black leading-tight tracking-[-0.033em]">Joined Contests</h1>
            <p className="text-[#ad92c9] text-base font-normal">Track your submissions, payment status, and upcoming deadlines.</p>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-[#261933] border-b border-[#362348]">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold text-[#ad92c9] uppercase tracking-wider text-xs">Payment Status</th>
                  ...
                  <th className="px-6 py-4 font-semibold text-[#ad92c9] uppercase tracking-wider text-xs text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#362348]">
                {/* Single Table Row */}
                {participated.map(p => <tr className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className="bg-orange-500/10 text-orange-300 border border-orange-500/20 px-2 py-1 rounded text-xs font-bold inline-flex items-center gap-1">
                      <span className="material-symbols-outlined text-[14px]">{p.status}</span>
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button className="text-white hover:text-[#7311d4] transition-colors text-sm font-medium">Submit Entry</button>
                  </td>
                </tr>)}
              </tbody>
            </table>
